...
type Int64Schema struct {
	...
	LimitValidator *Int64LimitValidatorSchema `hcl:"limit_validator,block"`
}
...
func (s *Int64ArraySchema) Validate(model *ModelSchema) error {
	if !ValidLabel.MatchString(s.Name) {
		return fmt.Errorf("invalid %s.int64_array name: %s", model.Name, s.Name)
	}
...
func (s *Int64MapSchema) Validate(model *ModelSchema) error {
	if !ValidLabel.MatchString(s.Name) {
		return fmt.Errorf("invalid %s.int64_map name: %s", model.Name, s.Name)
	}
